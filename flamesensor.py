...
class FlameSensor:
    ...
    def is_candle_in_range(self, candle, player_position):
        """Check if the flame sensor is in range of a candle."""
        sensor_x = player_position[0] + self.range * math.cos(math.radians(self.angle_offset))
        sensor_y = player_position[1] + self.range * math.sin(math.radians(self.angle_offset))
